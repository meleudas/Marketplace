...
public class ApiProductsControllerTests
{
    ...
    [Fact]
    [Trait("Suite", "API")]
    public async Task Create_Sends_CreateProductCommand()
    {
        var sender = new RecordingSender
        {
        ...
        var controller = BuildController(sender);
        var request = new UpsertProductRequest("N", "n", "d", 10, null, 0, 1, false, null, null);

        var result = await controller.Create(Guid.NewGuid(), request, CancellationToken.None);

        Assert.IsType<OkObjectResult>(result);
        Assert.IsType<CreateProductCommand>(sender.LastRequest);
    }

    [Fact]
    [Trait("Suite", "API")]
    public async Task Update_Sends_UpdateProductCommand()
    {
        var sender = new RecordingSender
        {
            NextResult = Result<ProductDto>.Success(new ProductDto(
        ...
        var result = await controller.Update(Guid.NewGuid(), 11, request, CancellationToken.None);

        Assert.IsType<OkObjectResult>(result);
        Assert.IsType<UpdateProductCommand>(sender.LastRequest);
    }
