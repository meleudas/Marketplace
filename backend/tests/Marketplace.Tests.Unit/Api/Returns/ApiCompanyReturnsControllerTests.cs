...
[Trait("Suite", "API")]
[Trait("Suite", "Returns")]
public sealed class ApiCompanyReturnsControllerTests
{
    [Fact]
    public async Task List_Sends_Query()
    {
        ...
        var result = await controller.List(Guid.NewGuid(), null, CancellationToken.None);

        Assert.IsType<OkObjectResult>(result);
        Assert.IsType<ListCompanyReturnsQuery>(sender.LastRequest);
    }
    ...
    public async Task Get_Sends_Query()
    {
        ...
        _ = await controller.Get(Guid.NewGuid(), 1, CancellationToken.None);

        Assert.IsType<GetReturnByIdQuery>(sender.LastRequest);
    }

    private static CompanyReturnsController BuildController(ISender sender)
    {
        var identity = new ClaimsIdentity([new Claim("sub", Guid.NewGuid().ToString())], "test");
        return new CompanyReturnsController(sender, new NoopIdempotency())
        {
