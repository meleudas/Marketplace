...
public class ApiInventoryControllerTests
{
    ...
    [Fact]
    public async Task Reserve_Sends_ReserveStockCommand()
    {
        var sender = new RecordingSender
        {
        ...
        var result = await controller.Reserve(
            companyId,
            new ReserveStockRequest(10, 100, 2, "reservation-1", 15, "cart-1"),
            CancellationToken.None);

        Assert.IsType<OkResult>(result);
        var command = Assert.IsType<ReserveStockCommand>(sender.LastRequest);
        Assert.Equal(companyId, command.CompanyId);
        Assert.Equal("reservation-1", command.ReservationCode);
        Assert.Equal(2, command.Quantity);
    }

    [Fact]
    public async Task GetWarehouses_WithoutUserId_Returns_Unauthorized()
    {
        var sender = new RecordingSender();
        var controller = new InventoryController(sender)
        {
        ...
        var result = await controller.GetWarehouses(Guid.NewGuid(), CancellationToken.None);
