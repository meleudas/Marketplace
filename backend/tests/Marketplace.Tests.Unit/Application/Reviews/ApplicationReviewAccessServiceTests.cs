...
public sealed class ApplicationReviewAccessServiceTests
{
    ...
    [Fact]
    public async Task ReplyAsCompany_Allows_Admin_Without_Membership()
    {
        var companyId = CompanyId.From(Guid.NewGuid());
        var actorId = Guid.NewGuid();
        var service = new ReviewAccessService(new StubCompanyMemberRepository(null));

        var allowed = await service.HasCompanyAccessAsync(companyId, actorId, true, ReviewPermission.ReplyAsCompany);

        Assert.True(allowed);
    }

    [Fact]
    public async Task ReplyAsCompany_Denies_When_No_Membership()
    {
        var companyId = CompanyId.From(Guid.NewGuid());
        var actorId = Guid.NewGuid();
        var service = new ReviewAccessService(new StubCompanyMemberRepository(null));
